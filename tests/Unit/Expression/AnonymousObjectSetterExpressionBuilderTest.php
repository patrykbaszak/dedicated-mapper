...
class AnonymousObjectSetterExpressionTester
{
    public string $test;
    public bool $test2;
...
class NestedAnonymousObjectSetterExpressionTester
{
    public string $test;
    public bool $test2;
    public AnonymousObjectSetterExpressionTester $test0;

    /** @var AnonymousObjectSetterExpressionTester[] */
...
/** @group unit */
class AnonymousObjectSetterExpressionBuilderTest extends TestCase
{
    ...
    protected function assertIsOutputAsigned(string $key): void
    {
        ...
        if (!$key[0] && !$key[1] && !$key[3] && !$key[4]) {
            $class = AnonymousObjectSetterExpressionTester::class;
            $property = 'test';
            $data = (object) [
                $property => 'test',
            ];
            ...
            $class = NestedAnonymousObjectSetterExpressionTester::class;
            $property = 'test0';
            $data = (object) [
                $property => (object) [
                    'test' => 'test',
    ...
    protected function assertIsAsignedSimpleObjectDeconstructor(string $key): void
    {
        ...
        $args = array_merge($key, [AnonymousObjectSetterExpressionTester::class, 'test3']);
        $expression = $this->getExpression(...$args);
        eval($expression);
